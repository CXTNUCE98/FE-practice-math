        ...
        <div class="lg:col-span-3">
          ...
          <div v-if="activeTab === 'upload'" class="card p-8 animate-fade-in-top">
            <h2 class="text-2xl font-bold mb-8 text-slate-900 dark:text-white flex items-center gap-3">
              <span class="w-10 h-10 bg-sky-100 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400 rounded-xl flex items-center justify-center">
            ...
            <form @submit.prevent="handleUpload" class="space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  ...
                  <input 
                    ...
                    type="text" 
                    placeholder="Ví dụ: Đề thi HK1 Toán lớp 12"
                ...
                <div class="space-y-2">
                  <label class="block text-sm font-bold text-slate-500 uppercase tracking-widest">Thời gian làm bài (Phút)</label>
                  <input 
                    v-model.number="examData.duration"
                    type="number" 
                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-2 focus:ring-sky-500 transition-all font-medium"
                    required
                  />
              ...
              <div class="space-y-2">
                <label class="block text-sm font-bold text-slate-500 uppercase tracking-widest">File Đề thi (.docx)</label>
                <div 
                ...
                >
                  ...
                  <div v-if="!examData.file" class="space-y-4">
                    <div class="w-20 h-20 bg-slate-50 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto text-3xl text-slate-400 group-hover:bg-sky-100 group-hover:text-sky-500 transition-all">
                  ...
                  <div v-else class="space-y-4">
                    <div class="w-20 h-20 bg-sky-100 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 rounded-full flex items-center justify-center mx-auto text-3xl">
                      <i class="bx bx-file"></i>
                    </div>
                    <div>
                      <p class="text-lg font-bold text-sky-600 dark:text-sky-400">{{ examData.file.name }}</p>
                      <button @click.stop="examData.file = null" class="text-sm text-red-500 font-bold hover:underline">Xóa và chọn lại</button>
                    </div>
                  </div>
                </div>
              </div>

              <div v-if="error" class="bg-red-50 dark:bg-red-900/20 text-red-500 p-4 rounded-xl border border-red-100 dark:border-red-800 text-sm">
                {{ (error as any)?.message || 'Tải lên không thành công. Vui lòng kiểm tra định dạng file.' }}
              </div>

              <div v-if="isSuccess" class="bg-green-50 dark:bg-green-900/20 text-green-600 p-4 rounded-xl border border-green-100 dark:border-green-800 text-sm font-bold">
                Tải lên và phân tích đề thi thành công!
              </div>

              <button 
                type="submit" 
                :disabled="isPending || !examData.file"
            ...
            <div class="overflow-hidden border border-slate-100 dark:border-slate-800 rounded-2xl">
              <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                  <tr v-for="exam in exams" :key="exam.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all">
                    <td class="px-6 py-4">
