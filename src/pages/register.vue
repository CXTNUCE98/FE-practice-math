...
<template>
  <div class="min-h-[80vh] flex items-center justify-center px-4 bg-slate-50 dark:bg-slate-900">
    <div
      class="max-w-md w-full space-y-8 p-8 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700">
      <div v-if="isSuccess" class="text-center space-y-4">
        <div
          class="w-16 h-16 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full flex items-center justify-center mx-auto text-3xl">
      ...
      <div v-else>
        ...
        <form class="mt-8 space-y-6" @submit.prevent="handleRegister">
          <div class="space-y-4">
            <div>
              <label for="full-name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Họ và
                tên</label>
            ...
            <div>
              <label for="class-name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Lớp</label>
              <input id="class-name" v-model="registerData.className" name="className" type="text"
                class="mt-1 block w-full px-4 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:border-transparent outline-none transition-all"
            ...
            <div>
              <label for="password" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Mật
                khẩu</label>
          ...
          <div v-if="error"
            class="text-red-500 text-sm bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-800">
